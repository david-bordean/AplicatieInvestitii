...
static void afiseazaMeniuPortofoliu(const Investitor& inv) {
    ...
    std::cout << "Profit total:   " << inv.profitTotal()      << " RON\n";
...
int main() {
    ...
    std::deque<Investitor> investitori;
    Piata& piata = Piata::getInstance();
    int optiune = -1;
    ...
    while (optiune != 0) {
        ...
        if (optiune == 1) {
            ...
            if (exista) {
                ...
                piata.inregistreaza(&investitori.back());  // inregistreaza la Observer
            ...
            piata.actualizeazaPreturi();  // notifica automat toti investitorii (Observer)
