...
class Investitor : public IObserverPiata {
    std::string nume;
    std::vector<InstrumentFinanciar*> portofoliu;
    std::vector<Tranzactie<double>> istoricTranzactii;
    double soldLiber;
...
public:
    ...
    void notificaActualizarePret(const Piata& p) override;  // IObserverPiata
