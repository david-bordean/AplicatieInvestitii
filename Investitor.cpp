...
void Investitor::cumpara(const InstrumentFinanciar& instr, int cantitate) {
    ...
    for (auto* p : portofoliu) {
        if (p->getNume() == instr.getNume()) {
            double pretMediuNou = (p->getPretCumparare() * p->getCantitate() +
                                   instr.getPretCurent() * cantitate) /
                                  (p->getCantitate() + cantitate);
            p->modificaCantitate(cantitate);
            p->setPretCumparare(pretMediuNou);
            soldLiber -= cost;
...
void Investitor::notificaActualizarePret(const Piata& p) {
    actualizeazaPreturiPortofoliu(p);
}
...
std::ostream& operator<<(std::ostream& out, const Investitor& inv) {
    ...
    out << "Sold liber: " << inv.soldLiber
        << " RON | Balanta totala: " << inv.getBalantaTotala()
        << " RON | Profit total: " << inv.profitTotal() << " RON\n";
